...
检测阶段默认启用 `response_format: json_object`，并要求模型输出顶层 JSON object，减少模型输出 Thinking Process/自然语言分析的概率。若 VLM 仍没有按 Label Studio JSON 输出，会抛出可识别的 JSON 解析错误，进入 direct pipeline 的重试队列。连续 3 次仍不合法时，该样本会被标记为 `discarded`，并在 `processed/retry_failures/<sample_id>.json` 写入失败原因，避免把“模型输出格式错误”误当成“图片里没有人”，从而生成无框场景图。
...
direct pipeline 对检测和分类共用一套 JSON 重试策略：
...
同一张样本连续 3 次出现模型 JSON 不合法，会写出：

```text
data/processed/metadata/<sample_id>.json          # workflow_status=discarded
data/processed/retry_failures/<sample_id>.json    # 失败原因和错误预览
```

`discarded` 样本不会进入 Label Studio 导出。
