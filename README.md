...
这个子模块要求大模型按 Label Studio 标准输出百分比框。`request_image_max_side` 只会压缩发给 VLM 的请求图片，metadata 仍然记录原图坐标；如果模型没有遵守提示而输出了 resize 后图片上的像素坐标，`coordinate_units: auto` 会识别大于 100 的像素值，并按请求图尺寸缩放回原图坐标：
...
如果你确认服务端总是返回 Label Studio 百分比坐标，可以改成 `coordinate_units: percent`；如果你接入的模型总是返回像素 `x/y/width/height`，可以改成 `coordinate_units: pixel`，pipeline 会按请求图片尺寸映射回原图。
