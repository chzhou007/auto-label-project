...
检测阶段默认 `fail_on_parse_error: true`：如果 VLM 没有按 Label Studio JSON 输出，会抛出可识别的 JSON 解析错误，进入 direct pipeline 的重试队列。连续 3 次仍不合法时，该样本会被标记为 `discarded`，并在 `processed/retry_failures/<sample_id>.json` 写入失败原因，避免把“模型输出格式错误”误当成“图片里没有人”，从而生成无框场景图。
